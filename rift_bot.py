...
MAX_RIFTS_PER_DISCORD_MESSAGE = 1
...
def build_rift_embed(server_id, rift_time, age, players, mode, target_epoch):
    ...
    if mode == "incoming":
        target_time = format_time(age + rift_time)
        countdown = f"<t:{target_epoch}:R>"
        exact_time = f"<t:{target_epoch}:T>"
        description = "This server is the closest tracked Rift window."
    else:
        target_time = format_time(age - rift_time)
        countdown = "active now"
        exact_time = f"started {format_time(rift_time)} ago"
    ...
    while True:
        try:
            ...
            for server_id, rift_time, age, notify_cycle, players, mode in candidates[:MAX_RIFTS_PER_DISCORD_MESSAGE]:
                ...
                target_epoch = int(now + rift_time)
                embeds.append(build_rift_embed(server_id, rift_time, age, players, mode, target_epoch))
